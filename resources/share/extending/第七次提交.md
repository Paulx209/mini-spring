...
### 
是这样的，我们会基于GenericConversionService类为基础，然后在他的基础上再添加用户自定义的转换器！要添加的时候，通过ConversionServiceFactoryBean类，提供一个converter，执行registerConverters方法即可！

那我们可以通过xml配置文件进行添加，在xml配置文件中注入ConversionServiceFactoryBean类和Converter类！或者是通过@Component注解添加！

添加转换器的时候会在ApplicationContext中的实例化所有的bean方法中添加！

然后在属性注入和value注解的时候使用即可！
